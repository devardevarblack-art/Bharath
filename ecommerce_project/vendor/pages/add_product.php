...
if ($edit_id) {
    $product = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM products WHERE id=$edit_id AND vendor_id=$vendor_id LIMIT 1"));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$error) {
        if ($edit_id) {
            mysqli_query($conn, "UPDATE products SET name='$name', description='$desc', price=$price, original_price=" . ($orig_price === 'NULL' ? 'NULL' : $orig_price) . ", stock=$stock, category_id=" . ($category_id === 'NULL' ? 'NULL' : $category_id) . ", status='$status', image='" . mysqli_real_escape_string($conn,$image_val) . "' WHERE id=$edit_id AND vendor_id=$vendor_id");
            $msg = 'Product updated successfully!';
            $product = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM products WHERE id=$edit_id LIMIT 1"));
        } else {
            mysqli_query($conn, "INSERT INTO products (vendor_id, category_id, name, description, price, original_price, stock, image, status) VALUES ($vendor_id, " . ($category_id === 'NULL' ? 'NULL' : $category_id) . ", '$name', '$desc', $price, " . ($orig_price === 'NULL' ? 'NULL' : $orig_price) . ", $stock, '" . mysqli_real_escape_string($conn,$image_val) . "', '$status')");
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        ...
        <p class="page-subtitle"><?php echo $edit_id ? 'Update product details' : 'List a new product in your store'; ?></p>
    </div>
        ...
        <form method="POST" enctype="multipart/form-data">
            <div class="row g-3">
                ...
                <div class="col-12">
                    ...
                    <textarea name="description" class="form-control" rows="3" placeholder="Describe your product..."><?php echo htmlspecialchars($product['description'] ?? ''); ?></textarea>
                </div>
                <div class="col-md-3">
                ...
                <div class="col-md-6">
                    ...
                    <div class="mt-2">
                        <?php if ($edit_id && !empty($product['image'])): ?>
                            <img src="../uploads/products/<?php echo htmlspecialchars($product['image']); ?>" id="imgPreview" class="show" alt="Product">
                        <?php else: ?>
